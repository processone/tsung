...
PostgreSQL, MySQL, LDAP, and Jabber servers.
...
web traffic and SOAP),  Webdav, Jabber and Postgresql are currently
...
\item Erlang/OTP R10B-0 and up
  ...
  \item mochiweb libs (for xpath parsing, optionaly used for dynamic variables in
    the HTTP plugin):
    sources available at
...
\section{HTTP/WebDAV benchmark approach}
...
The recorder has three plugins: for HTTP, WABDAV and for PostgreSQL.
...
function to parse a simple csv file: you cau use \varname{setdynvars}
...
\strong{Since version 1.3.0}, more powerful variables is implemented:
FIXME
...
Figure \ref{fig:report} show an exemple of a summary report.
\begin{figure}[htb]
  \begin{center}
...
Figure \ref{fig:graph} show an exemple of a graphical report.
...
\begin{description}
\item[error\_closed]
\item[error\_inet\_XXX] \url{http://www.erlang.org/doc/man/inet.html}
\item[error\_unknown\_data]
\item[error\_unknown\_msg]
\item[error\_unknown]
\item[error\_repeat\_<REPEATNAME>]
\item[error\_send\_XXX]
\item[error\_send]
\item[error\_connect\_XXX]
\item[error\_no\_online jabber]
\item[error\_no\_offline jabber]
\item[error\_no\_free\_userid]
\item[error\_next\_session]
\item[error\_mysql\_<ERRNO>]
\item[error\_mysql\_badpacket]
\item[error\_pgsql]
\end{description}
