...
\item  connected (number of simultaneous connected users) \strong{new
  in 1.2.2}
...
\item \varname{request\_noack} (counter of \varname{no\_ack} requests. Since
  ...
  requests, we keep a separate stats for this. \strong{This was
...
\item \varname{bidi\_msg\_skip} only if bidi is true for a
  ...
  without doing anything.  \strong{This was
    introduced in version 1.2.2} )
\item \varname{bidi\_msg\_sent} only if bidi is true for a
  ...
  of a message received from from the server. \strong{This was
    introduced in version 1.2.2} )
\end{itemize}
